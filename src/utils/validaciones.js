...
export const validarDominioCorreo = (email) => {
...
export const validarFormularioRegistro = (formData) => {
  ...
  // Validar RUT
    ...
    } else if (formData.rut.length < 7 || formData.rut.length > 9) {
    errores.rut = '❌ El RUT debe tener entre 7 y 9 caracteres';
...
export const validarFormularioContacto = (formData) => {
    ...
    if (formData.email?.trim() && formData.email.length > 100) {
    errores.email = '❌ Máximo 100 caracteres';
    } else if (formData.email?.trim() && !validarDominioCorreo(formData.email)) {
